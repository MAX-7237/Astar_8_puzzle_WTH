...
def linear_conflict(state):
    ...
    for r in range(N):
        ...
        for c in range(N):
            idx = r * N + c
            value = state[idx]
            if value != 0 and goal_pos[value] // N == r:
                goal_c = goal_pos[value] % N
    ...
    for c in range(N):
        ...
        for r in range(N):
            idx = r * N + c
            value = state[idx]
            if value != 0 and goal_pos[value] % N == c:
                goal_r = goal_pos[value] // N
...
def manhattan_plus_linear_conflict(state):
...
def build_result_text(results):
    ...
    lines.append("实验 3：A* 算法不同估价函数对比")
    lines.append("=" * 70)
    lines.append("")
    ...
    lines.append("一、实验对比结果")
    lines.append("-" * 70)
    lines.append("| 估价函数 | 是否找到解 | 解的步数 | 扩展节点数 | 运行时间/s |")
    lines.append("| --- | --- | ---: | ---: | ---: |")
    ...
    for result in results:
        ...
        if path is None:
            lines.append(f"| {name} | 否 | - | {expanded} | {elapsed:.6f} |")
        else:
            lines.append(f"| {name} | 是 | {len(path) - 1} | {expanded} | {elapsed:.6f} |")

    lines.append("")
    lines.append("二、各估价函数详细求解过程")
    lines.append("=" * 70)
    ...
    lines.append("三、实验分析")
    lines.append("=" * 70)
    lines.append("错位数字数只统计有多少数字没有到达目标位置，信息量较少，因此通常扩展节点更多。")
    lines.append("曼哈顿距离考虑每个数字到目标位置的行列距离，估价更准确，搜索效率通常更高。")
    lines.append("曼哈顿距离加线性冲突进一步考虑同行或同列数字之间的顺序冲突，启发信息更强，通常可以减少扩展节点。")
    lines.append("本程序同时生成 exp3_heuristic_comparison.svg 对比图，以及每种估价函数对应的贪吃蛇路径可视化 SVG 文件。")
...
def create_comparison_svg(results, output_svg_path):
    ...
    svg_parts.append(f'<rect x="0" y="0" width="{width}" height="{height}" fill="#eef5ff"/>')
    svg_parts.append(svg_text(width / 2, 40, "实验 3：不同估价函数 A* 搜索效果对比", size=22, weight="bold"))
    ...
    for metric_name, center_x, max_value in metrics:
        ...
        for i, result in enumerate(results):
            ...
            svg_parts.append(svg_text(x + bar_width / 2, y - 8, f"{value:.3f}" if metric_name == "time" else int(value), size=12))
...
def main():
    heuristic_methods = [
        ("错位数字数", misplaced_tiles),
        ("曼哈顿距离", manhattan_distance),
        ("曼哈顿距离 + 线性冲突", manhattan_plus_linear_conflict),
    ]

    results = []

    for name, func in heuristic_methods:
        t0 = time.time()
        path, expanded = astar(start, goal, func)
        t1 = time.time()
    ...
    txt_path = current_dir / "exp3_multi_heuristic_result.txt"
    comparison_svg_path = current_dir / "exp3_heuristic_comparison.svg"
    ...
    for result in results:
        ...
        path_svg_path = current_dir / f"exp3_path_{name}.svg"
        create_path_svg(
            result["path"],
            path_svg_path,
            f"实验 3：{result['name']} 的 A* 求解路径",
            cols=6,
        )
